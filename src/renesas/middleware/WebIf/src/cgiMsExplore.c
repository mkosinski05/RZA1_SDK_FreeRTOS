 ...
 *              : 13.11.2019 1.01    Fixed issue SSGRZAISRC-686. Web MS Drive.
 ******************************************************************************/
...
static int bdGeneratePage (PHTDIR pHtDir)
{
    ...
    if (pHtDir->pDrive)
    {
        ...
        _Bool bfNext = false;
        _Bool bfRoot = true;
        char chLocaldir[BD_CLIENT_PATH_LENGTH];
        ...
        if (iReady)
        {
            ...
            /* Directory path must have the drive specifier at the start e.g. 'A:' */
            chLocaldir[0] = pHtDir->chDrive;
            chLocaldir[1] = ':';
            chLocaldir[2] = 0;
            strcat (chLocaldir,pHtDir->pszDir);

            pHtDir->fatError = R_FAT_FindFirst(&gs_dir, &pHtDir->fatEntry, chLocaldir, "*");
