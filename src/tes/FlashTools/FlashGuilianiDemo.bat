...
echo Bootloader for RSK+ RZ/A1  = 2
...
cd JLink
"%BASE%\JLink.exe" -if JTAG -speed 12000 -device R7S721001 -jtagconf -1,-1 -CommanderScript FlashBootloader_StreamIt.Command
pause
cd ..
goto :loop
...
pause
cd JLink
"%BASE%\JLink.exe" -if JTAG -speed 12000 -device R7S721001_DualSPI -jtagconf -1,-1 -CommanderScript FlashApplication_Release_StreamIt_eGML.Command
pause
cd ..
goto :loop
...
cd JLink
cls
"%BASE%\JLink.exe" -if JTAG -speed 12000 -device R7S721001_DualSPI -jtagconf -1,-1 -CommanderScript FlashResources_StreamIt.Command
pause
cd ..
goto :loop
