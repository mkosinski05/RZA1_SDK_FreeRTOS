...
set KEYNAME=HKEY_LOCAL_MACHINE\SOFTWARE\SEGGER\J-Link
...
echo.
if %JVER% GEQ %MINJVER% (goto JTAGCONF_CHECK)
chgclr 0C
echo ===================================================================
...
rem * Above search code gratefully provided by TES Guiliani *
    ...
    echo.    
    echo using SET_MACRONIX
    echo bootloader module 
    echo using SET_MACRONIX_DDR_DUAL_4B
    set SelectedBootFile=StreamIt2_QSPI_Loader_DDR_DUAL_4B.bin 
   ...
   "%programmer%\%programmer2%" -speed 12000 -if JTAG -JTAGConf -1,-1 -device R7S721031 -CommanderScript LoadUserBoot.Command
